...
def balanced(m):
    ...
    rarm = right(m)
    rlength = length(rarm)
    ltotal_mass = total_mass(end(larm))
    rtotal_mass = total_mass(end(rarm))
    if is_mobile(end(larm)):
        if balanced(end(larm)) == False:
            return False
    if is_mobile(end(rarm)):
        if balanced(end(rarm)) == False:
            return False
    if llength * ltotal_mass == rtotal_mass * rlength:
        return True
    return False
...
def max_path_sum(t):
    ...
    maxvalue = label(t)
    for i in branches(t):
        outcome = max_path_sum(i)
        if outcome + label(t) > maxvalue:
            maxvalue = outcome + label(t)
    return maxvalue
